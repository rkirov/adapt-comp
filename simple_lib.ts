...
export function makeIncList<T>(arr: T[]): IncrList<T> {
  ...
  return comp(pure({value: arr[0], tail: makeIncList(arr.slice(1))})) as IncrList<T>;
}
...
export function map<T,S>(l: IncrList<T>, f: (x:T) => S): IncrList<S> {
  return comp(read(l, pair => {
    if (!pair) return pure(null);
    return pure({value: f(pair.value), tail: map(pair.tail, f)});
  }));
}

export function reduce<T,S>(l: IncrList<T>, f: (acc: S, x:T) => S, initial: S): Modifiable<S> {
  return comp(read(l, pair => {
